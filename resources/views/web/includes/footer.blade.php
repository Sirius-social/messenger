<footer class="bg-slate-900 py-20 lg:px-0 px-4">
    <div class="container mx-auto sm:flex sm:space-y-0 space-y-6">
        <div class="flex-none sm:w-1/2">
            <div class="footer-column">
                <div class="flex items-center text-slate-100 font-bold text-xl sm:text-3xl mb-4">
                    <img class="w-16 h-16 mr-2" src="{{ asset('images/logo/1@3x.png') }}" alt="Networks Synergy">
                    Sirius Messenger
                </div>
                <p class="text-slate-500 text-lg mb-20 pr-12">
                    Donec sollicitudin molestie malesuada. Mauris blandit aliquet elit, eget tincidunt
                    nibh pulvinar a. Vestibulum ac diam sit amet quam vehicula elementum sed sit amet dui. Curabitur non
                    nulla
                    sit amet nisl tempus convallis quis ac lectus. Proin eget tortor risus. Cras ultricies ligula sed
                    magna
                    dictum porta. Vestibulum ac diam sit amet quam vehicula elementum sed sit amet dui.
                </p>
                <div class="footer-apps flex-none inline-grid px-4 gap-x-6">
                    <a href="#" class="flex flex-col items-center justify-center text-sm font-semibold mb-6 text-slate-300 text-slate-600 hover:text-slate-500">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 305 305"
                             class="w-12 h-12 mb-6" xml:space="preserve"><path
                                d="M40.738 112.119c-25.785 44.745-9.393 112.648 19.121 153.82C74.092 286.523 88.502 305 108.239 305c.372 0 .745-.007 1.127-.022 9.273-.37 15.974-3.225 22.453-5.984 7.274-3.1 14.797-6.305 26.597-6.305 11.226 0 18.39 3.101 25.318 6.099 6.828 2.954 13.861 6.01 24.253 5.815 22.232-.414 35.882-20.352 47.925-37.941 12.567-18.365 18.871-36.196 20.998-43.01l.086-.271a2.5 2.5 0 0 0-1.328-3.066c-.032-.015-.15-.064-.183-.078-3.915-1.601-38.257-16.836-38.618-58.36-.335-33.736 25.763-51.601 30.997-54.839l.244-.152a2.498 2.498 0 0 0 .71-3.511c-18.014-26.362-45.624-30.335-56.74-30.813a50.043 50.043 0 0 0-4.95-.242c-13.056 0-25.563 4.931-35.611 8.893-6.936 2.735-12.927 5.097-17.059 5.097-4.643 0-10.668-2.391-17.645-5.159-9.33-3.703-19.905-7.899-31.1-7.899-.267 0-.53.003-.789.008-26.03.383-50.626 15.275-64.186 38.859z"/>
                            <path
                                d="M212.101.002c-15.763.642-34.672 10.345-45.974 23.583-9.605 11.127-18.988 29.679-16.516 48.379a2.499 2.499 0 0 0 2.284 2.164c1.064.083 2.15.125 3.232.126 15.413 0 32.04-8.527 43.395-22.257 11.951-14.498 17.994-33.104 16.166-49.77a2.515 2.515 0 0 0-2.587-2.225z"/>
                        </svg>
                        IOS
                    </a>
                    <a href="#" class="flex flex-col items-center justify-center text-sm font-semibold mb-6 text-slate-300 text-slate-600 hover:text-slate-500">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 299.679 299.679"
                             class="w-12 h-12 mb-6" xml:space="preserve"><path
                                d="M181.122 299.679c10.02 0 18.758-8.738 18.758-18.758v-43.808h12.525c7.516 0 12.525-5.011 12.525-12.525V99.466H74.749v125.123c0 7.515 5.01 12.525 12.525 12.525H99.8v43.808c0 10.02 8.736 18.758 18.758 18.758 10.019 0 18.756-8.738 18.756-18.758v-43.808h25.051v43.808c-.001 10.019 8.737 18.757 18.757 18.757zM256.214 224.589c10.02 0 18.756-8.737 18.756-18.758v-87.615c0-9.967-8.736-18.75-18.756-18.75-10.021 0-18.758 8.783-18.758 18.75v87.615c0 10.02 8.736 18.758 18.758 18.758zM43.466 224.589c10.021 0 18.758-8.737 18.758-18.758v-87.615c0-9.967-8.736-18.75-18.758-18.75-10.02 0-18.756 8.783-18.756 18.75v87.615c0 10.02 8.736 18.758 18.756 18.758zM209.899 1.89c-2.504-2.52-6.232-2.52-8.736 0l-16.799 16.743-.775.774c-9.961-4.988-21.129-7.479-33.566-7.503l-.182-.002h-.002l-.184.002c-12.436.024-23.604 2.515-33.564 7.503l-.777-.774L98.516 1.89c-2.506-2.52-6.232-2.52-8.736 0-2.506 2.506-2.506 6.225 0 8.729l16.25 16.253c-5.236 3.496-9.984 7.774-14.113 12.667-9.885 11.717-16.19 26.966-17.057 43.488-.008.172-.025.342-.033.514a72.074 72.074 0 0 0-.078 3.391H224.93c0-1.135-.027-2.266-.078-3.391-.008-.172-.025-.342-.035-.514-.865-16.522-7.172-31.772-17.057-43.487-4.127-4.893-8.877-9.171-14.113-12.667l16.252-16.253c2.506-2.505 2.506-6.224 0-8.73zm-91.365 63.173a9.383 9.383 0 1 1 0-18.766 9.383 9.383 0 0 1 0 18.766zm62.611 0a9.383 9.383 0 1 1 0-18.766 9.383 9.383 0 0 1 0 18.766z"/>
                        </svg>
                        Android
                    </a>
                    <a href="#" class="flex flex-col items-center justify-center text-sm font-semibold mb-6 text-slate-300 text-slate-600 hover:text-slate-500">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 305 305" class="w-12 h-12 mb-6" xml:space="preserve">
                            <path
                                d="M139.999 25.775v116.724a2.5 2.5 0 0 0 2.5 2.5H302.46a2.5 2.5 0 0 0 2.5-2.5V2.5A2.5 2.5 0 0 0 302.1.026L142.139 23.301a2.5 2.5 0 0 0-2.14 2.474zM122.501 279.948a2.498 2.498 0 0 0 2.5-2.5V162.5a2.5 2.5 0 0 0-2.5-2.5H2.592a2.5 2.5 0 0 0-2.5 2.5l.006 98.515a2.5 2.5 0 0 0 2.16 2.477l119.903 16.434c.113.014.227.022.34.022zM2.609 144.999h119.892a2.5 2.5 0 0 0 2.5-2.5V28.681a2.502 2.502 0 0 0-2.836-2.477L2.164 42.5A2.5 2.5 0 0 0 0 44.98l.109 97.521a2.501 2.501 0 0 0 2.5 2.498zM302.46 305a2.5 2.5 0 0 0 2.5-2.5l.04-140a2.5 2.5 0 0 0-2.5-2.5H142.499a2.5 2.5 0 0 0-2.5 2.5v117.496c0 1.246.918 2.302 2.151 2.476l159.961 22.504c.117.016.233.024.349.024z"/>
                        </svg>
                        Windows
                    </a>
                </div>
            </div>
        </div>
        <div class="flex sm:w-1/2">
            <div class="w-1/2">
                <h5 class="font-semibold text-slate-100">Product</h5>
                <nav class="footer-navigation nav">
                    <ul class="mt-3 space-y-2">
                        <li class="nav-item">
                            <a href="{{ route('web.features') }}" class="hover:text-slate-300 text-slate-400 text-sm leading-6">
                                Features
                            </a>
                        </li>
                        <li class="nav-item">
                            <a href="{{ route('web.downloads') }}" class="hover:text-slate-300 text-slate-400 text-sm leading-6">
                                Download
                            </a>
                        </li>
                        <li class="nav-item">
                            <a href="#" class="hover:text-slate-300 text-slate-400 text-sm leading-6">
                                Security
                            </a>
                        </li>
                    </ul>
                </nav>
            </div>
            <div class="w-1/2">
                <h5 class="font-semibold text-slate-100">Support</h5>
                <nav class="footer-navigation nav">
                    <ul class="mt-3 space-y-2">
                        <li class="nav-item">
                            <a href="#" class="hover:text-slate-300 text-slate-400 text-sm leading-6">
                                Blog
                            </a>
                        </li>
                        <li class="nav-item">
                            <a href="#" class="hover:text-slate-300 text-slate-400 text-sm leading-6">
                                Help center
                            </a>
                        </li>
                    </ul>
                </nav>
            </div>
        </div>
    </div>
</footer>
